#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Persistent registry for user-uploaded ONNX audio classification models."""
...
def _get_registry_path():
    ...
    if getattr(sys, "frozen", False):
        ...
        return get_registry_path("audio_models_registry.json")
    return os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "audio_models_registry.json"
    )
...
def load_registry() -> list:
    """Return all registry entries whose ONNX file still exists on disk."""
    entries = _load_raw()
    valid = []
    for e in entries:
...
def save_entry(entry: dict) -> None:
    """Insert or update (by name) a registry entry and persist to disk."""
    entries = _load_raw()
    updated = False
...
def remove_entry(name: str) -> None:
    """Remove an entry by name from the registry."""
    entries = _load_raw()
