...
def image_process(image, color_space):
    """Convert image to different color spaces for color-based object detection.
    
    Color space conversion is essential for robust object detection under varying
    lighting conditions. HSV is ideal for color-based detection, YCrCb for skin
    detection, and LAB for perceptual color operations.
    ...
    conversions = {
        0: None,  # BGR (no conversion)
        1: cv2.COLOR_BGR2HSV,  # HSV
        2: cv2.COLOR_BGR2YCrCb,  # YCrCb
        3: cv2.COLOR_BGR2LAB,  # LAB
...
class FactoryNode:
    ...
    def add_node(
        ...
        node.tag_node_combo_name = node.tag_node_name + ':ColorSpaceCombo'
        node.tag_node_combo_value_name = node.tag_node_name + ':ColorSpaceComboValue'
        ...
        with dpg.node(
            ...
            with dpg.node_attribute(
                ...
                dpg.add_combo(
                    ...
                    label='Color Space',
                    items=node._color_spaces,
                    default_value=node._color_spaces[0],
...
class Node(Node):
    ...
    _color_spaces = ['BGR', 'HSV', 'YCrCb', 'LAB', 'HLS']
    ...
    def update(
        ...
        combo_tag = tag_node_name + ':ColorSpaceComboValue'
        ...
        color_space_str = dpg_get_value(combo_tag)
        color_space = self._color_spaces.index(color_space_str)
        ...
        if frame is not None and enable_processing:
            frame = image_process(frame, color_space)
    ...
    def get_setting_dict(self, node_id):
        ...
        color_space = dpg_get_value(combo_tag)
        enable_value = dpg_get_value(enable_checkbox_tag)

        setting_dict = {}
        setting_dict['ver'] = self._ver
        setting_dict['pos'] = pos
        setting_dict[combo_tag] = color_space
    ...
    def set_setting_dict(self, node_id, setting_dict):
        ...
        if combo_tag in setting_dict:
            color_space = setting_dict[combo_tag]
            dpg_set_value(combo_tag, color_space)
