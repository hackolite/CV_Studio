#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ComfortAgent — LLM-driven comfort management via OpenRouter.

Shares the same architecture as AmbianceAgent; specialised by its system
prompt which focuses on thermal comfort, air quality, and ergonomics.
"""

from node.AgentNode.node_ambiance_agent import Node as AmbianceAgentNode

_AGENT_TYPE = 'ComfortAgent'

_SYSTEM_PROMPT = (
    "You are an expert comfort engineer. "
    "Analyse the provided sensor data (temperature, humidity, air quality, occupancy, etc.) "
    "and user prompt, then select and configure the available tools to optimise the "
    ...
    def _build_messages(self, data, prompt, tools):
        ...
        user_content = {
            ...
            'instruction': (
                ...
                'and with which parameters to optimise comfort. '
