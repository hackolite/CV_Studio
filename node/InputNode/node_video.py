...
class VideoNode(Node):
    ...
    def _generate_frame_spectrogram(self, frame):
        """
        Generate a spectrogram from a video frame using 2D FFT.
        
        Args:
            frame: OpenCV BGR image (video frame)
            
        Returns:
            BGR image representing the frequency spectrum of the frame
        """
        if frame is None:
            return None
        ...
        try:
            ...
            magnitude_normalized = (magnitude_spectrum - magnitude_spectrum.min()) / (magnitude_spectrum.max() - magnitude_spectrum.min() + 1e-6)
            ...
            spectrum_colored = cmap(magnitude_normalized)
            
            # Convert to 8-bit RGB (remove alpha channel)
            spectrum_rgb = (spectrum_colored[:, :, :3] * 255).astype(np.uint8)
            
            # Convert to BGR for OpenCV/DPG compatibility
            spectrum_bgr = cv2.cvtColor(spectrum_rgb, cv2.COLOR_RGB2BGR)
            
            return spectrum_bgr
            
        except Exception as e:
            print(f"Failed to generate frame spectrogram: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def update(
        ...
        if dpg.does_item_exist(tag_node_spectrogram_toggle):
            ...
            if show_spectrogram and frame is not None:
                # Generate spectrogram directly from the current frame
                spectrogram_bgr = self._generate_frame_spectrogram(frame)
    ...
    def _callback_file_select(self, sender, data):
        if data['file_name'] != '.':
            ...
            # No need to prepare spectrogram - it will be generated from each frame
