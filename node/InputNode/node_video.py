...
class VideoNode(Node):
    ...
    def __init__(self):
        ...
        # NOUVEAU : Stockage optimisé avec cache LRU
        self._full_audio = {}  # Audio complet en mémoire (léger)
        self._spectrogram_cache = {}  # Cache LRU pour spectrogrammes
        self._cache_max_size = 50  # Garde 50 spectros max (~5 MB)
    ...
    def _preprocess_video(self, node_id, movie_path, chunk_duration=5.0, step_duration=0.1):
        """
        Version optimisée : charge SEULEMENT l'audio, pas de pré-calcul des spectrogrammes
        """
    ...
    def _save_audio_as_mp3(self, node_id, movie_path, y, sr):
        ...
        try:
            ...
            try:
                ...
                print(f"💾 Audio sauvegardé : {audio_mp3_path}")
            except subprocess.CalledProcessError:
                audio_wav_path = os.path.join(video_dir, f"{video_basename}_audio.wav")
                sf.write(audio_wav_path, y, sr)
                print(f"💾 Audio sauvegardé : {audio_wav_path}")
            finally:
                if os.path.exists(tmp_wav_path):
                    os.unlink(tmp_wav_path)
        except Exception as e:
            print(f"⚠️ Erreur sauvegarde audio : {e}")
    
    def _get_spectrogram_for_frame(self, node_id, frame_number):
        """
        Version optimisée : génère spectrogrammes à la volée avec cache LRU
        """
    ...
    def _compute_spectrogram_for_chunk(self, node_id, chunk_index):
        ...
        # Générer spectrogramme (même code qu'avant)
        binsize = 2**10
    ...
    def _prefetch_next_spectrograms(self, node_id, current_chunk_index):
        """Pré-calcule intelligemment les 3 prochains chunks"""
        metadata = self._chunk_metadata[node_id]
    ...
    def update(
        ...
        if dpg.does_item_exist(tag_node_spectrogram_toggle):
            ...
            if show_spectrogram and str(node_id) in self._full_audio:
                ...
                # Get spectrogram for this frame (lazy loaded with cache)
