...
def test_affine_head_is_used_for_training():
    """Without a PyTorch backprop path, train_step routes to the affine
    correction head and reports its updates."""
    tr = _make_trainer(training_active=True)
    teacher = [[20, 20, 60, 60]]
    raw = np.array([[38, 38, 78, 78]], dtype=np.float32)
    tr.infer = lambda f: (raw.copy(),
    ...
    assert res['training_step'] is True
    assert tr.get_stats()['adapter_updates'] == 1
    assert tr.backprop_mode == "affine-head"
