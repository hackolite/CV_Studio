...
1. **Python 3.7 ou supérieur** (3.10 ou 3.11 recommandé)
   - Python 3.7 à 3.12 sont officiellement supportés par CV Studio
   - Python 3.8+ est recommandé pour une expérience optimale avec toutes les dépendances
...
1. **Téléchargez Python** depuis le site officiel :
   ...
   - Téléchargez la dernière version de Python 3.11 ou 3.10 pour Windows (ou Python 3.8+ pour une meilleure expérience)
...
> **Note** : Si vous rencontrez une erreur concernant le paquet `lap` qui nécessite numpy lors de l'installation, vous pouvez l'installer manuellement d'abord :
> ```cmd
> pip install numpy>=1.21.0
> pip install -r requirements.txt
> ```
> Cela garantit que numpy est disponible avant les paquets qui en ont besoin lors de la compilation.
...
- Essayez de réinstaller : `pip install --upgrade dearpygui`
- Ou utilisez la version spécifiée dans requirements.txt : `pip install -r requirements.txt --force-reinstall`
