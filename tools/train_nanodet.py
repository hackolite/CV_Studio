...
Améliorations production :
  ...
  - LR scheduler cosine avec warmup linéaire
...
class Head(nn.Module):
    ...
    def __init__(self, num_classes: int) -> None:
        super().__init__()
        self.shared = ConvBnRelu(128, 128, kernel=3, padding=1)
...
def train(cfg: Config = CFG) -> None:
    ...
    transform = T.Compose([
        T.RandomResizedCrop((cfg.img_size, cfg.img_size), scale=(0.5, 1.0)),
    ...
    _warmup_epochs = min(5, max(1, cfg.epochs // 20))
    cosine_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=max(1, cfg.epochs - start_epoch - _warmup_epochs), eta_min=cfg.lr * 1e-2
    )
