...
"""
PyInstaller spec file for CV_Studio with internal directory structure
...
The .exe will be created in the root directory (next to internal/).
"""
...
# Collect data files from internal directory
datas = []

# Add node directory with all subdirectories and files
datas.append((os.path.join(internal_path, 'node'), 'node'))

# Add node_editor directory
datas.append((os.path.join(internal_path, 'node_editor'), 'node_editor'))

# Add src directory
datas.append((os.path.join(internal_path, 'src'), 'src'))
...
a = Analysis(
    ...
    pathex=[internal_path, base_path],
    binaries=binaries,
    datas=datas,
